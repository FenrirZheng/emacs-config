...
The 16 modules under [`lisp/`](lisp/) (`init-defaults`, `init-system-packages`, `init-completion`, `init-corfu`, `init-snippets`, `init-editing`, `init-languages`, `init-git`, `init-terminal`, `init-appearance`, `init-dirvish`, `init-org`, `init-org-roam`, `init-ai`, `init-aidermacs`, `init-tmux-claude`) mostly correspond to one section of the pre-2026-05-19 monolithic `init.el` (see `git log --oneline` for the split commits); `init-aidermacs` and `init-tmux-claude` are later standalone additions. Section numbers in module headers refer to that pre-split layout — they're history, not a current TOC; read the module names instead.
