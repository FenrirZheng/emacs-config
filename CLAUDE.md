...
- **`eglot-booster` monkey-patches `eglot--connect`**. An Emacs upgrade may break it; recover at runtime with `M-x eglot-booster-mode` (toggles off). Built-in Eglot is the default LSP client here for every language **except Java** — see the next bullet.
- **Java is the lsp-mode exception**: `java-mode` / `java-ts-mode` are hooked to `lsp-deferred` (lsp-mode), not Eglot. The `lsp-mode` + `lsp-java` + `consult-lsp` + `dap-mode` stack lives in the dedicated "Java exception" block of [`lisp/init-languages.el`](lisp/init-languages.el). Rationale: `eclipse.jdt.ls` is heavy enough that references / workspace-symbol latency is dominated by the client's partial-result + cancellation support, which lsp-mode has and Eglot doesn't. `lsp-java` also pre-wires the JDT settings that matter for references speed (`includeDecompiledSources=nil`, `includeAccessors=nil`, `-Xmx3G` vmargs) and ships `dap-java` for debugging. The booster integration (`emacs-lsp-booster`) is the same Rust binary the Eglot side uses, wrapped via two `advice-add` forms in that block. **Do not add more lsp-mode hooks** beyond Java without revisiting this architecture note — every other language stays on Eglot.
- **Java multi-project monorepo workspace** (jdtls multi-root): jdtls is registered with `:multi-root t`, so one jdtls process serves multiple project roots through a single Eclipse workspace at [`var/lsp-java/workspace/`](var/lsp-java/workspace/) — cross-project find-references / rename / workspace-symbol all just work. For a Maven monorepo with **no top-level aggregator pom** (sibling top-level projects each with their own `pom.xml`, e.g. `~/code/hitok2/` whose `im-combined-api`, `im-combined-hitok`, `hitok-java-backend` sit as siblings), the clean recipe is: register **the monorepo root** as a single `lsp-session-folder` via `M-x lsp-workspace-folders-add RET <monorepo-root> RET` — jdtls boots with that root as `rootUri`, scans for every `pom.xml` underneath, and imports each as a JDT project into the same workspace. `lsp-workspace-folders-add` auto-dedupes by removing descendants of the new root from `lsp-session-folders` (persisted to [`var/lsp/session.el`](var/lsp/session.el)), so the previous per-sub-project entries vanish automatically. **Don't register deep sub-modules** (e.g. `<repo>/<top>/biz/<starter>/`) as session folders — `lsp-find-session-folder` uses `--first` semantics, so an over-deep root shadows the intended parent and pins jdtls to that single sub-module, silently breaking cross-project find-references. Symptom: `(lsp-workspaces)` in the misbehaving buffer shows only the deep path; fix is `M-x lsp-workspace-folders-remove RET <deep-path> RET` then reopen. To inspect live state: `(lsp-session-folders (lsp-session))` lists the registered roots, `(lsp--session-workspaces (lsp-session))` lists the actually-running jdtls workspaces (multi-root means one workspace covers all roots — if you see more than one workspace per server-id, multi-root failed and `~/.emacs.d/var/lsp-java/workspace/` likely needs a full reset).
- **DAP debugging is `dape`, not `dap-mode`, *except for Java*** — `dape` (GNU ELPA; the `use-package` block at the end of [`lisp/init-languages.el`](lisp/init-languages.el)) renders breakpoints in the buffer **margin** (`B` glyph), visible on TTY frames; `dap-mode` draws them with GUI-only fringe bitmaps (invisible in `emacsclient -nw`). The Java exception: `dap-mode` + `dap-java` are installed alongside `lsp-java` because `dape` has no usable Java adapter; the trade-off is that breakpoint markers won't show in TTY frames. Java debugging is therefore intended for **GUI Emacs frames only**. Don't generalise dap-mode to any other language — dape remains the TTY-safe debugger for Go / Python / etc. Debug-adapter binaries live outside the repo: only Go's `dlv` is on `PATH`; `debugpy` / `gdb` / `codelldb` / `vscode-js-debug` / the Java debug + test bundles install per language as needed. Keybindings are catalogued in [FEATURES.md §7](FEATURES.md).
